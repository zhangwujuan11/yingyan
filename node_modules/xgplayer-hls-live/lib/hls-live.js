...
var HlsLiveController = function () {
  ...
  _createClass(HlsLiveController, [{
    ...
    value: function init() {
      ...
      this._context.registry('TS_BUFFER', _xgplayerHelperModels.Buffer);
      this._context.registry('TRACKS', _xgplayerHelperModels.Tracks);

      this._playlist = this._context.registry('PLAYLIST', _xgplayerHelperModels.Playlist)({ autoclear: true });
      this._context.registry('PRE_SOURCE_BUFFER', _xgplayerHelperModels.PreSource);

      this._context.registry('COMPATIBILITY', _xgplayerHelperCodec.Compat);

      // 初始化M3U8Loader;
      this._m3u8loader = this._context.registry('M3U8_LOADER', _xgplayerHelperUtils.FetchLoader)({ buffer: 'M3U8_BUFFER', readtype: 1 });
      this._tsloader = this._context.registry('TS_LOADER', _xgplayerHelperUtils.FetchLoader)({ buffer: 'TS_BUFFER', readtype: 3 });

      // 初始化TS Demuxer
      this._context.registry('TS_DEMUXER', _xgplayerHelperTransmuxers.TsDemuxer)({ inputbuffer: 'TS_BUFFER' });

      // 初始化MP4 Remuxer
      this._context.registry('MP4_REMUXER', _xgplayerHelperTransmuxers.Mp4Remuxer);

      // 初始化MSE
      this.mse = this._context.registry('MSE', _xgplayerHelperUtils.Mse)({ container: this._player.video });
      this.initEvents();
    }
  }, {
    key: 'initEvents',
      ...
      if (buffer.TAG === 'M3U8_BUFFER') {
        ...
        try {
          ...
          if (result && result[2]) {
            ...
            mdata = _xgplayerHelperTransmuxers.M3U8Parser.parse(this.m3u8Text, this.baseurl);
          }
        } catch (error) {
    ...
    value: function _checkStatus() {
      var container = this._player.video;
      ...
      if (container.buffered.length < 1) {
        ...
        var bufferstart = container.buffered.start(container.buffered.length - 1);
        ...
        var bufferend = container.buffered.end(container.buffered.length - 1);
    ...
    value: function load(url) {
      this.baseurl = _xgplayerHelperTransmuxers.M3U8Parser.parseURL(url);
      this.url = url;
      this._preload();
    }
  }, {
